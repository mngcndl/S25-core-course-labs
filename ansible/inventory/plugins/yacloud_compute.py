...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def verify_file(self, path):
        if super(InventoryModule, self).verify_file(path):
            if path.endswith(('default_yacloud_compute.yml', 'yacloud_compute.yaml')):
                return True
        display.debug(
            "yacloud_compute inventory filename must end with 'default_yacloud_compute.yml' or 'yacloud_compute.yaml'")
        return False
